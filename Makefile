...
src/bootstrap.js: deps client/src/cell.js client/src/cell-pluginBuilder.js
	...
	find ./client/src ./server.coffee -name '*.coffee' -type f | xargs $(npmbin)/coffee -c -b 
	node $(requirejsBuild) \
		-o \
...
# DEV 
#------------------------------------------------------------------- 
server: server.coffee deps
	$(npmbin)/coffee server.coffee

dev-stylus: deps
	find ./client/src -name '*.styl' -type f | xargs $(npmbin)/stylus --include client/src/shared/styles --watch --compress

dev-coffee: deps
	find ./client/src ./server.coffee -name '*.coffee' -type f | xargs $(npmbin)/coffee -c -b --watch
